...
bool Table2by2::MustExactTest() {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      expected2by2[i][j] =
          (double)m_marginRow[i] * (double)m_marginCol[j] / (double)m_sum;
...
void Table2by2::ChisqTest(bool &valid) {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      chisqStat += ((double)m_n[i][j] - expected2by2[i][j]) *
                   ((double)m_n[i][j] - expected2by2[i][j]) /
                   expected2by2[i][j];
      //                      printf("expected2by2 = %f,
      //                      %f\n",expected2by2[i][j], ((double) m_n[i][j] -
      //                      expected2by2[i][j])*( (double) m_n[i][j] -
      //                      expected2by2[i][j])/expected2by2[i][j]);
    }
  }
...
void Table2by2::FisherExactTest() {
  ...
  double pObserved = gsl_sf_choose(m_marginRow[0], m_n[0][0]) *
                     gsl_sf_choose(m_marginRow[1], m_n[1][0]) /
                     gsl_sf_choose(m_sum, m_marginCol[0]);
  pExact = 0;
  //      printf("n1+ = %d, n11 = %d, sum = %d, pObserved =
  //      %f\n",m_marginCol[0], m_n[0][0], m_sum, pObserved);
  //      printf("lowerbound = %d, upperbound = %d\n",lowerbound, upperBound);
  if (upperBound != lowerBound) {
    for (int i = lowerBound; i <= upperBound; i++) {
      if (i != m_n[0][0]) {
        double pPossible = gsl_sf_choose(m_marginRow[0], i) *
                           gsl_sf_choose(m_marginRow[1], m_marginCol[0] - i) /
                           gsl_sf_choose(m_sum, m_marginCol[0]);
...
void Table2by2::GeneralTest() {
  ...
  /*      printf("valid to use pearson chisq test?
...
void Table2by2::initLogFacs(double *logFacs, int n) {
  ...
  for (int i = 1; i < n + 1; ++i) {
    logFacs[i] =
        logFacs[i - 1] + log((double)i);  // only n times of log() calls
  }
}
...
double Table2by2::logHypergeometricProb(double *logFacs, int a, int b, int c,
                                        int d) {
  return (logFacs[a + b] + logFacs[c + d] + logFacs[a + c] + logFacs[b + d] -
          logFacs[a] - logFacs[b] - logFacs[c] - logFacs[d] -
          logFacs[a + b + c + d]);
}
...
void Table2by2::FullFastFisherExactTest() {
  ...
  for (int i = lowerBound; i <= upperBound; i++) {
    double logpPossible = logHypergeometricProb(
        storeLogFacs, i, m_marginRow[0] - i, m_marginCol[0] - i,
...
double Table2by2::GetpChisq() { return (pChisq); }

double Table2by2::GetpExact() { return (pExact); }

double Table2by2::GetstatChisq() { return (chisqStat); }

double Table2by2::GetpGeneral() { return (pGeneral); }
...
double Table2by2::GetpFasterFisherExact(const char *testIndicator) {
  if (strcmp(testIndicator, "TwoSided") == 0) {
    ...
    error(
        "Inappropriate test side specification! 0: two sided; 1: one sided "
        "greater; -1: one sided less\n");
  }
