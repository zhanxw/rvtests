...
class FirthRegression {
 public:
  ...
  // main function
  bool FitFirthModel(Matrix& X, Matrix& y,
                     int rnrounds);  // return false if not converging
  bool FitFirthModel(Matrix& X, Vector& y,
                     int rnrounds);  // return false if not converging
  bool FitFirthModel(Matrix& X, Vector& succ, Vector& total, int nrrounds);

  // alias simplified functions
  bool Fit(Matrix& X, Matrix& y) { return this->FitFirthModel(X, y, 100); }
  bool Fit(Matrix& X, Vector& y) { return this->FitFirthModel(X, y, 100); }
