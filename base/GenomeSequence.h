...
class Chromosome {
 public:
  explicit Chromosome(FILE* faFile, Faidx::Info* info)
      : fp(faFile), info(info){};
 ...
 public:
  ...
  int size() const { return info->contigSize; };
...
class GenomeSequence {
 ...
 public:
   ...
   */
  int size() const { return this->faidx.size(); };
  /**
   * @return total number of chromosome
   */
  long int getGenomeLength() const { return faidx.getGenomeLength(); };
  ...
  bool exists(const std::string& c) {
    if (this->data.find(c) != this->data.end()) return true;
    if (faidx.getInfo(c) != NULL) return true;
    return false;
  }
