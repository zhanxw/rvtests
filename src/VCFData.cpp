...
void VCFData::writeGenotype(const char* fn) {
  this->writeTable(fn, this->genotype, this->marker2Idx, this->people2Idx,
                   "MarkerName");
};
// void VCFData::writeCollapsedGenotype( const char* fn){
//     this->writeTable(fn, this->collapsedGenotype, this->people2Idx,
//     this->set2Idx, "PeopleID");
// };
...
void VCFData::writeCovariate(const char* fn) {
  this->writeTable(fn, this->covariate, this->people2Idx, this->covariate2Idx,
                   "PeopleID");
};
void VCFData::writePhenotype(const char* fn) {
  this->writeTable(fn, this->phenotype, this->people2Idx, this->phenotype2Idx,
                   "PeopleID");
};
 ...
 */
void VCFData::writeTable(const char* fn, Matrix* data,
 ...
 * value instead.
 */
int VCFData::readTable(const char* fn, Matrix* data,
                       OrderedMap<std::string, int>* rowName,
                       OrderedMap<std::string, int>* colName,
                       std::string* upperLeftName, double defaultValue) {
  if (!data || data->rows == 0 || data->cols == 0 || !upperLeftName) return -1;
  ...
  while (lr.readLineBySep(&fd, " \t")) {
    ...
    if (lineNo == 1) {
      ...
      if (fd.size() == header.size()) {
        ...
        for (int r = 1; r < header.size(); r++) (*rowName)[header[r]] = r - 1;
      } else {
        // first column does not have header
        for (int r = 0; r < header.size(); r++) (*rowName)[header[r]] = r;
      }
    }
...
int VCFData::readPlinkTable(const char* fn, Matrix* data,
                            OrderedMap<std::string, int>* rowName,
                            OrderedMap<std::string, int>* colName,
                            double defaultValue) {
  if (!fn || !data) return -1;
 ...
 */
int VCFData::readPlinkPhenotypeSkipMissing(
    const char* fn, const char* selectedCol, Matrix* data,
    OrderedMap<std::string, int>* rowName,
    OrderedMap<std::string, int>* colName) {
  if (!fn || !data) return -1;
