...
// }
void ParameterParser::AddParameter(ParameterType pt, void* data, const char* flag,
                                   const char* doc) {
  std::string f = flag;
...
void ParameterParser::InitializeValue(ParameterType pt, void* data, const char* flag) {
  // initialize data
  switch (pt) {
    case BOOL_TYPE:
      *(bool*)data = false;
...
void ParameterParser::WriteToFileWithComment(const char* fileName, const char* comment) {
 ...
 */
void ParameterParser::WriteToStreamWithComment(std::ostream& fout, const char* comment) {
  if (strlen(comment) > 0) {
    fout << "# " << comment << "\n";
  } else {
    char username[128] = "unknown_user";
