...
bool LogisticRegressionScoreTest::FitLogisticModel(Matrix& X, Vector& y,
                                                   int colToTest, int nRound) {
  Matrix Xnull;
  Vector Xcol;
  ...
  //     return false;
  // }
  if (!this->FitNullModel(Xnull, y, nRound)) return false;
  if (!this->TestCovariate(Xnull, y, Xcol)) return false;
  return true;
}

bool LogisticRegressionScoreTest::FitNullModel(Matrix& Xnull, Vector& y,
                                               int nRound) {
 ...
 * V =  ( \sum _i v_i S_i S_i^T - (\sum v_i Z_i S_i^T) T  inv(\sum v_i Z_i
