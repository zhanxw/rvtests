...
// const static char errorMessage1[] = "Completion with error > abseps";
// // inform = 1
// const static char errorMessage2[] = "N greater 1000 or N < 1"; // inform = 2
// const static char errorMessage3[] ="Covariance matrix not positive
// semidefinite";  // inform = 3
// const static char* errorMessage[4] = {errorMessage0, errorMessage1,
 ...
 * @return CDF of multivariate normal P ( lower < X < upper ) where X ~ MVN(0,
 * correlationMatrix)
 * @return p-value, or -1.0 if error happens
 */
double MvtNorm::compute_Band(
    int n, double* lower, double* upper,
...
/**
 * @return CDF of multivariate normal P ( X < bound ) where X ~ MVN(0,
 * correlationMatrix)
 */
double MvtNorm::compute_P(
    int n, double* bound,
    double* correlationMatrix)  // (2,1), (3,1), (3,2) .....
{
  lower.resize(n);
...
/**
 * @return (1 - CDF) of multivariate normal P ( X > bound ) where X ~ MVN(0,
 * correlationMatrix)
 */
double MvtNorm::compute_Q(
    int n, double* bound,
    double* correlationMatrix)  // (2,1), (3,1), (3,2) .....
{
  upper.resize(n);
 ...
 * @return 0 if succeed
 */
int MvtNorm::compute(int* n, double* lower, double* upper, int* infin,
                     double* correl) {
#ifdef DEBUG
  const int N = *n;
  ...
  mvtdst_(n, &this->nu, lower, upper, infin, correl, delta.data(),
          &this->maxpts, &this->abseps, &this->releps, &this->error,
          &this->prob, &this->inform);
...
const char* MvtNorm::getError(int i) {
  const int n =
      sizeof(MvtNorm::informMessage) / sizeof(MvtNorm::informMessage[0]);
