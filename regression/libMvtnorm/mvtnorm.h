...
class MvtNorm {
 public:
   ...
   */
  double compute_Band(int n, double* lower, double* upper,
   ...
   * @return p-value, or -1.0 if error happens
   */
  double compute_P(int n, double* bound,
                   double* correlationMatrix  // (2,1), (3,1), (3,2) .....
                   );
   ...
   * @return p-value, or -1.0 if error happens
   */
  double compute_Q(int n, double* bound,
                   double* correlationMatrix  // (2,1), (3,1), (3,2) .....
                   );

  const char* getError(int i);
  double getAbsEps() const { return this->abseps; }

 private:
  /**
   * @return <0 if anything goes wrong.
   */
  int compute(int* n, double* lower, double* upper, int* infin, double* correl);
 ...
 private:
  ...
  // static const char errorMessage1[] = "Completion with error > abseps";
  // // inform = 1
  // static const char errorMessage2[] = "N greater 1000 or N < 1";
  // // inform = 2
  // static const char errorMessage3[] ="Covariance matrix not positive
  ...
  // parameters
  int nu;  // the number of degrees of freedom, if nu < 0, then use multivariate
           // normal.
