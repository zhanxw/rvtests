...
ADD_DOUBLE_PARAMETER(minGQ, "--minGQ", "Specify the minimum genotype "
...
int main(int argc, char **argv) {
  ...
  if (pout)
    pout->writeHeader(vin.getVCFHeader());
  int lineNo = 0;
  ...
  while (vin.readRecord()) {
    ...
    VCFIndividual *indv;
    if (FLAG_passFilter && PassFilter != r.getFilt())
      continue;
  ...
  if (incompatibleChrom) {
    fprintf(stdout, "Skipped %d variants that are not located on PLINK "
  ...
  if (lowMAF) {
    fprintf(stdout, "Skipped %d sites due to Minor Allele Frequency "
                    "lower than %f\n",
            lowMAF, FLAG_minMAF);
  }
  if (highMAF) {
