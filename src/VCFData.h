 ...
 *      markerName (id), markerChrom, markerPos, markerRef, markerAlt,
 * markerFreq
 *      peopleName
...
class VCFData {
 public:
   ...
   */
  void adjustPeopleOrder(std::vector<std::string>& p,
                         std::vector<std::string>* droppedId) {
    std::map<int, std::string>
        order;  // will adjust internal order of phenotype and covariate.
    std::map<int, std::string>::iterator order_it;

    int nDropped = 0;
    for (int i = 0; i < p.size(); i++) {
      if (!this->people2Idx.find(p[i])) {  // VCF file has p[i], but not in
                                           // existing phenotype or covariate.
        nDropped++;
        if (droppedId) droppedId->push_back(p[i]);
      } else {
    ...
    for (int i = 0; i < order.size(); i++) {
      ...
      if (i == j)
        ...
        if (this->phenotype->rows > j) this->phenotype->SwapRows(i, j);
        if (this->covariate->rows > j) this->covariate->SwapRows(i, j);
      }
    }
    fprintf(stdout,
            "%d samples from VCF files will not be included in study.\n",
            nDropped);
    this->people2Idx.clear();
    for (int i = 0; i < order.size(); i++) this->people2Idx[order[i]] = i;
  };
  ...
  void addVCFHeader(VCFHeader* h, std::vector<std::string>* droppedId) {
    ...
    if (this->people2Idx.size() == 0) {
      ...
      for (int i = 0; i < v.size(); i++) this->people2Idx[v[i]] = i;
      return;
    }
  ...
  int loadPhenotypeByCol(
      const char* fn,
      int col) {  // by default PLINK use 3rd column as phenotype
    // if this->genotype already have people2Idx, then make phenotype people
    // label match (also in order)
    // otherwise, just set people2Idx like the phenotype.
    if (this->people2Idx.size()) {
      OrderedMap<std::string, int> pName;
      int ret = this->readPlinkPhenotypeSkipMissing(
          fn, 0, this->phenotype, &pName, &this->phenotype2Idx);
      if (ret) {
        fprintf(
            stderr,
            "%d phenotype are skipped as their pheontype are not numeric.\n",
            ret);
      }
      int skip1, skip2;
      this->matchData(this->genotype, this->people2Idx, &skip1, this->phenotype,
                      pName, &skip2);
      if (skip1) {
        fprintf(stderr,
                "%d individuals' genotypes are dropped because of mismatch.\n",
                skip1);
      }
      if (skip2) {
        fprintf(stderr,
                "%d individuals' phenotype are dropped because of mismatch.\n",
                skip2);
      }
      return ret;
    } else {
      int ret = this->readPlinkPhenotypeSkipMissing(
          fn, 0, this->phenotype, &this->people2Idx, &this->phenotype2Idx);
      if (ret) {
        fprintf(
            stderr,
            "%d phenotype are skipped as their pheontype are not numeric.\n",
  ...
  int loadCovariate(const char* fn) {
    ...
    // if this->genotype already have people2Idx, then make covariate people
    // label match (also in order)
    // otherwise, just set people2Idx like the covariate.
    if (this->people2Idx.size()) {
      OrderedMap<std::string, int> pName;
      int ret =
          this->readPlinkTable(fn, this->covariate, &pName,
                               &this->covariate2Idx, defaultMissingCovariate);
      int skip1, skip2;
      this->matchData(this->genotype, this->people2Idx, &skip1, this->covariate,
                      pName, &skip2);
      if (skip1) {
        fprintf(stderr,
                "%d individuals' genotypes are dropped because of mismatch.\n",
      ...
      if (skip2) {
        fprintf(stderr,
                "%d individuals' covariates are dropped because of mismatch.\n",
                skip2);
      }
      return ret;
    } else {
      int ret =
          this->readPlinkTable(fn, this->covariate, &this->people2Idx,
                               &this->covariate2Idx, defaultMissingCovariate);
      if (ret) {
  ...
  // loop through rowLabel1, and switch the command rowLabel to the toppest
  // position
  void matchData(Matrix* m1, OrderedMap<std::string, int>& rowLabel1,
                 int* skip1, Matrix* m2,
                 OrderedMap<std::string, int>& rowLabel2, int* skip2) {
    assert(skip1 && skip2);
    ...
    for (unsigned int i = 0; i < rowLabel1.size(); i++) {
      ...
      if (rowLabel2.find(key1)) {
        ...
        if (nextRow1 != i) m1->SwapRows(nextRow1, i);
        nextRow1++;
        if (nextRow2 != i) m2->SwapRows(nextRow2, j);
        nextRow2++;
      } else {
   ...
   *   @param defaultValue, when data cannot be converted to double, use this
   * value instead.
   */
  int readTable(const char* fn, Matrix* data,
                OrderedMap<std::string, int>* rowName,
                OrderedMap<std::string, int>* colName,
                std::string* upperLeftName, double defaultValue);
  /**
   * similar to readTable, but skip the first column and
   * require header file have the same number of fields as other lines.
   */
  int readPlinkTable(const char* fn, Matrix* data,
                     OrderedMap<std::string, int>* rowName,
                     OrderedMap<std::string, int>* colName,
                     double defaultValue);
  // read genotypes.
  ...
  void readGenotypeFromR(const char* fn) {
    ...
    while (lr.readLineBySep(&fd, "\t ")) {
      if (!lineNo) {  // header line
        ...
        this->genotype->Dimension(this->marker2Idx.size(),
                                  this->people2Idx.size());
  ...
  void writeTable(const char* fn, Matrix* data,
                  OrderedMap<std::string, int>& rowName,
                  OrderedMap<std::string, int>& colName,
  ...
  void calculateFrequency(const int type) {
    ...
    for (int m = 0; m < numMarker; m++) {
      ...
      if (this->markerTotalAllele[m] > 0)
        this->markerFreq[m] =
            (double)(this->markerMAC[m]) / this->markerTotalAllele[m];
      else
  ...
  bool isCaseControlPhenotype() {
    ...
    if (wrongCoding > 0) {
      fprintf(stderr,
              "Case control coding should be 0, 1, or -9, but wrong coding "
              "detected.\n");
    }
    return true;
  };
  ...
  void dumpSize() {
    ...
    printf("collapsedGenotype: %d x %d ;\n", collapsedGenotype->rows,
           collapsedGenotype->cols);
 ...
 public:
  ...
  OrderedMap<std::string, int>
      marker2Idx;  // markerName -> idx in this->genotype
  // int numMarker;

  OrderedMap<std::string, int>
      covariate2Idx;  // covariate name -> idx in this->covariate
  OrderedMap<std::string, int>
