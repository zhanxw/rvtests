...
int main(int argv, const char** argc) {
  ...
  const char CSI2_MAGIC[] = "CSI\2";
  if (strncmp(magic, CSI2_MAGIC, 4)) {
    fprintf(stderr,
            "CSI2 magic is not found -- the input file is not CSI version 2 "
            "index!\n");
  }
  ...
  for (int32_t ref_idx = 0; ref_idx < n_ref; ++ref_idx) {
    ...
    for (int32_t i = 0; i < n_bin; ++i) {
      ...
      READ_WRITE_NUMBER(uint64_t, n_rec);
      READ_WRITE_NUMBER(uint64_t, n_chunk);
      for (uint64_t j = 0; j != n_chunk; ++j) {
