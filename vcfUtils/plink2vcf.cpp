...
int laodReference(const std::string& FLAG_reference, std::map<std::string, char>* reference){
  ...
  while (lr.readLineBySep(&fd, "\t ")) {
    ...
    if (fd.size() < 2 || fd[1].size() != 1) {
      fprintf(stderr, "Incorrect columns number or alleles length not equal to 1\n");
      continue;
    };
...
int main(int argc, char *argv[])
{
  ...
  BEGIN_PARAMETER_LIST(pl)
      ...
      ADD_STRING_PARAMETER(pl, reference, "--reference", "reference file (format: rsNumber reference_base), can be generated by getMapRef")
